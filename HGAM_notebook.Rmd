...
```{r}
# transform numbers per 100_m3 to KG using above conversions
fish.b$`73_ich`=fish.b$`73_ich`*1e-6
#fish.b$`73_ich`=ceiling(fish.b$`73_ich`*1e-6)
fish2.b=fish.b %>% pivot_longer(c(`73_Adt`, `73_Juv`, `73_ich`), names_to = c("SVSPP", "Stg"), names_sep ="_", values_to = "Biomass")
...
pred.test=predict(fish_modGS4d,dat.test,type='response')                  # make prediction for test set
preds.obs=data.frame(pred.test=pred.test,dat.test$pa)           # data frame of preds and obs   
colnames(preds.obs)=c("predicted","observed")
plotROC(preds.obs$observed,preds.obs$predicted, colorize = TRUE)
