...
load('/home/ryan/Git/NEhabitat/Final_merged_fish_corBIO_Zoo_Ich.Rda') # Abundance
...
# sample <- sample.int(n = nrow(fish), size = floor(.75*nrow(fish)), replace = F)
# trainPA <- fish[sample, ]
# testPA  <- fish[-sample, ]
...
fish2=fish %>% pivot_longer(c(`72_Adt`, `72_Juv`, `72_ich`), names_to = c("SVSPP", "Stg"), names_sep ="_", values_to = "Number")
...
set.seed(101) # Set Seed so that same sample can be reproduced in future also
# Now Selecting 75% of data as sample from total 'n' rows of the data  
sample <- sample.int(n = nrow(fish.b), size = floor(.75*nrow(fish.b)), replace = F)
trainBIO <- fish.b[sample, ]
testBIO  <- fish.b[-sample, ]

#fish.b$`73_ich`=ceiling(fish.b$`73_ich`*1e-6)
fish2.b=trainBIO %>% pivot_longer(c(`73_Adt`, `73_Juv`, `73_ich`), names_to = c("SVSPP", "Stg"), names_sep ="_", values_to = "Biomass")
fish2.b$Stg=factor(fish2.b$Stg, ordered=F)

## log transform plankton
fish3.b=fish2.b
logd.b=fish3.b[,8:17]
logd.b=log10(logd.b+1)
fish3.b[,8:17]=logd.b
...
test=fish4[which(fish4$Number>0),]
hist(test$Number[which(test$Stg=='ich')])
hist(test$Number[which(test$Stg=='Juv')])
hist(test$Number[which(test$Stg=='Adt')])

levs=seq(from=min(fish4$BOTTEMP), to=max(fish4$BOTTEMP), by=0.2)
barplot(table(factor(fish4$BOTTEMP, levels=levs)))

levs=seq(from=min(fish4$DEPTH), to=max(fish4$DEPTH), by=1)
barplot(table(factor(fish4$DEPTH, levels=levs)))

# check on zooplankton and... remove outliers(?)
hist(fish4$pseudo_100m3)
levs=sort(unique(fish4$pseudo_100m3))
levs2=levs[2:length(levs)]
barplot(table(factor(fish4$pseudo_100m3, levels=levs2)))
summary(fish4$pseudo_100m3)
#outliers...
test=rev(sort(fish4$pseudo_100m3))
barplot(test[1:1000])
which(test<500000)[1]
...
```{r}
fish_modG = gam(pa ~s(BOTTEMP, k=20, bs="tp") +s(SURFTEMP, k=20, bs='tp') + s(DEPTH, k=10, bs="tp") + s(Stg, k=4, bs='re') + s(pseudo_100m3, k=20, bs="cr") + s(grnszmm, k=10, bs='tp') + s(rug, k=20, bs='tp'), data=fish4, method = "REML", family="binomial")
```
...
```{r}
fish_modG2 = gam(pa ~s(BOTTEMP, k=20, bs="tp") +s(SURFTEMP, k=20, bs='tp') + s(DEPTH, k=10, bs="tp") + s(Stg, k=4, bs='re') + s(pseudo_100m3, k=20, bs="cr") + s(grnszmm, k=10, bs='tp') + s(rug, k=20, bs='tp'), data=fish4, method = "REML", family="tw")
par(mfrow=c(2,2))
...
```{r}
fish_modG3 = gam(pa ~s(BOTTEMP, k=20, bs="tp") +s(SURFTEMP, k=20, bs='tp') + s(DEPTH, k=10, bs="tp") + s(Stg, k=4, bs='re') + s(pseudo_100m3, k=20, bs="cr") + s(grnszmm, k=10, bs='tp') + s(rug, k=20, bs='tp'), data=fish4, method = "REML", family="quasipoisson")
...
```{r}
fish_modG4 = gam(pa ~ s(LON, LAT) + s(BOTTEMP, k=20, bs="tp") +s(SURFTEMP, k=20, bs='tp') + s(DEPTH, k=10, bs="tp") + s(Stg, k=4, bs='re') + s(pseudo_100m3, k=20, bs="cr") + s(grnszmm, k=10, bs='tp') + s(rug, k=20, bs='tp'), data=fish4, method = "REML", family="binomial")

gam.check(fish_modG4)
fish_modG4$aic
draw(fish_modG4)
# [1] 16241.43
pred.test=predict(fish_modG4,dat.test,type='response')                  # make prediction for test set
preds.obs=data.frame(pred.test=pred.test,dat.test$pa)           # data frame of preds and obs   
...
```{r}
# plotROC(preds.obs$dat.test,preds.obs$pred.test, colorize = TRUE)
plotROC(preds.obs$observed,preds.obs$predicted, colorize = TRUE)
...
dat.test.pts=SpatialPoints(cbind(dat.test$LON, dat.test$LAT),proj4string=CRS("+proj=longlat"))
dat.test.pts=spTransform(dat.test.pts, CRS("+proj=utm +zone=19 +datum=WGS84"))
dat.test$Northings=dat.test.pts@coords[,2]
dat.test$Eastings=dat.test.pts@coords[,1]
pred.test=predict(fish_modG4b,dat.test,type='response')                  # make prediction for test set
preds.obs=data.frame(pred.test=pred.test,dat.test$pa)           # data frame of preds and obs   
colnames(preds.obs)=c("predicted","observed")
...
pred.test=predict(fish_modG4bll,dat.test,type='response')                  # make prediction for test set
preds.obs=data.frame(pred.test=pred.test,dat.test$pa)           # data frame of preds and obs   
colnames(preds.obs)=c("predicted","observed")
plotROC(preds.obs$observed,preds.obs$predicted, colorize = TRUE)
...
```{r}
# fish_modG5 = gam(pa ~ s(LAT, LON) + s(BOTTEMP, k=20, bs="tp") +s(SURFTEMP, k=20, bs='tp') + s(DEPTH, k=10, bs="tp") + s(Stg, k=4, bs='re') + s(pseudo_100m3, k=20, bs="cr") + s(grnszmm, k=10, bs='tp') + s(rug, k=20, bs='tp'), data=fish4, method = "REML", family="binomial", select=T)
...
# fish_modG5 = gam(pa ~ s(LON, LAT) + s(BOTTEMP, k=20, bs="tp") +s(SURFTEMP, k=20, bs='tp') + s(DEPTH, k=10, bs="tp") + s(Stg, k=4, bs='re') + s(pseudo_100m3, k=20, bs="cr") +  s(rug, k=20, bs='tp'), data=fish4, method = "REML", family="binomial") #, select=T)
...
fish_modG4_bio = gam(logbio ~ s(LON, LAT) + s(BOTTEMP, k=20, bs="tp") +s(SURFTEMP, k=20, bs='tp') + s(DEPTH, k=20, bs="tp") + s(Stg, k=4, bs='re') + s(pseudo_100m3, k=20, bs="cr") + s(grnszmm, k=10, bs='tp') + s(rug, k=20, bs='tp'), data=fish4.b, method = "REML", family="gaussian")



fish_modG4b_bio = gam(logbio ~ s(LON, LAT) + s(BOTTEMP, k=20, bs="tp") +s(SURFTEMP, k=20, bs='tp') + s(DEPTH, k=20, bs="tp") + s(Stg, k=4, bs='re') + s(pseudo_100m3, k=20, bs="cr") + s(grnszmm, k=10, bs='tp') + s(rug, k=20, bs='tp'), data=fish4.b, method = "REML", family="gaussian")
...
```{r}
pred.test.bio=predict(fish_modG4_bio,dat.b.test,type='response')          # make prediction for test set
preds.bio.test=data.frame(pred.gam=(10^pred.test.bio)-1,true.bio=(10^dat.b.test$logbio)-1)
...
```{r}
fish_modGS4 = gam(pa ~ s(LON, LAT) + s(BOTTEMP, Stg, k=20, bs="fs") +s(SURFTEMP, Stg, k=20, bs='fs') + s(DEPTH, Stg, k=10, bs="fs") + s(pseudo_100m3, Stg, k=20, bs="fs") + s(grnszmm, Stg, k=10, bs='fs') + s(rug, Stg, k=20, bs='fs'), data=fish4, method = "REML", family="binomial") #, select=T)

fish_modGS4b = gam(pa ~ te(LON, LAT, Stg, bs = c("tp", "tp", "fs")) + s(BOTTEMP, Stg, k = 10, bs = "fs") + s(SURFTEMP, Stg, k = 10, bs = "fs") + s(DEPTH, Stg, k = 10, bs = "fs") + s(pseudo_100m3, Stg, k = 10, bs = "fs") + s(grnszmm, Stg, k = 10, bs = "fs") + s(rug, Stg, k = 10, bs = "fs")data=fish4, method = "REML", family="binomial") 
  
fish_modGS4c =  gam(pa ~ ti(LON, LAT, Stg, bs=c('fs')) + s(BOTTEMP, Stg, k=10, bs="fs") +s(SURFTEMP, Stg, k=10, bs='fs') + s(DEPTH, Stg, k=10, bs="fs") + s(pseudo_100m3, Stg, k=10, bs="fs") + s(grnszmm, Stg, k=10, bs='fs') + s(rug, Stg, k=10, bs='fs'), data=fish4, method = "REML", family="binomial")

fish_modGS4d =  gam(pa ~ s(LON, LAT) + s(BOTTEMP, Stg, k=10, bs="fs") +s(SURFTEMP, Stg, k=10, bs='fs') + s(DEPTH, Stg, k=10, bs="fs") + s(pseudo_100m3, Stg, k=10, bs="fs") + s(grnszmm, Stg, k=10, bs='fs') + s(rug, Stg, k=10, bs='fs'), data=fish4, method = "REML", family="binomial")

# add fs for stg to all vars
fish_modGS4e =  gam(pa ~ te(LON, LAT, bs='tp', m=2) +t2(LON, LAT, Stg, bs=c('tp', 'tp', 'fs'), m=2, full=T) + s(BOTTEMP, bs='tp') + s(BOTTEMP, Stg, k=10, bs="fs") + s(SURFTEMP, bs='tp') + s(SURFTEMP, Stg, k=10, bs='fs') + s(DEPTH, bs='tp') + s(DEPTH, Stg, k=10, bs="fs") + s(pseudo_100m3, bs='tp') + s(pseudo_100m3, Stg, k=10, bs="fs") + s(grnszmm, bs='tp') + s(grnszmm, Stg, k=10, bs='fs') + s(rug, bs='tp') + s(rug, Stg, k=10, bs='fs'), data=fish4, method = "REML", family="binomial")

fish_modGS4e$aic
summary(fish_modGS4d)$dev.expl
pred.test=predict(fish_modGS4e,dat.test,type='response')                  # make prediction for test set
preds.obs=data.frame(pred.test=pred.test,dat.test$pa)           # data frame of preds and obs   
colnames(preds.obs)=c("predicted","observed")
plotROC(preds.obs$observed,preds.obs$predicted, colorize = TRUE)
...
```{r}
fish_modGI4 =  gam(pa ~ s(LON, LAT) + s(LON, LAT, by=Stg) + s(BOTTEMP, k=10, bs="fs") + s(BOTTEMP, k=10, by=Stg, bs="fs") +s(SURFTEMP, Stg, k=10, bs='fs') + s(SURFTEMP, Stg, k=10, by=Stg, bs='fs') + s(DEPTH, Stg, k=10, bs="fs") + s(DEPTH, Stg, k=10, by=Stg, bs="fs") + s(pseudo_100m3, Stg, k=10, bs="fs") + s(pseudo_100m3, Stg, k=10, by=Stg, bs="fs")+ s(grnszmm, Stg, k=10, bs='fs') + s(grnszmm, Stg, k=10, by=Stg, bs='fs') + s(rug, Stg, k=10, bs='fs') +s(rug, Stg, k=10, by=Stg, bs='fs') + s(Stg, k=10, bs='fs'), data=fish4, method = "REML", family="binomial")
