...
Add variables from rasters to both biomass and numbers database --> no longer needed
```{r}
...
# plot(phi2mm)
# FData.abn$grnszmm = raster::extract(phi2mm, mypoints)
# FData.bio$grnszmm = raster::extract(phi2mm, mypoints)
# masked.raster=loadRData('/home/ryan/Git/NEhabitat/rast_mud_fraction.rdata')
# FData.abn$mud_pct = raster::extract(masked.raster, mypoints)
# FData.bio$mud_pct = raster::extract(masked.raster, mypoints)
# masked.raster=loadRData('/home/ryan/Git/NEhabitat/rast_sand_fraction.rdata')
# FData.abn$sand_pct = raster::extract(masked.raster, mypoints)
# FData.bio$sand_pct = raster::extract(masked.raster, mypoints)
...
# plot(rugscl)
# FData.abn$rug = raster::extract(rugscl, mypoints)
# FData.bio$rug = raster::extract(rugscl, mypoints)

## add chlorophyll climatology
# chllist=list.files('/home/ryan/1_habitat_analysis_2017/chl/NESREG/clim', pattern='.RData')
# wd3=paste('/home/ryan/1_habitat_analysis_2017/chl/NESREG/clim')
# chlclim=data.frame(matrix(data=NA, nrow=length(FData.abn[,1]), ncol=12))
# polylist=cbind(FData.abn$LON, FData.abn$LAT)
# polylist2=polylist[1:30,]
# for (i in 1:12){
# rastchl=loadRData(paste(wd3, '/', chllist[i], sep=''))
# chlclim[,i]=raster::extract(rastchl, polylist)
# }
# FData.abn$chl1=chlclim[,1]
# FData.abn$chl2=chlclim[,2]
# FData.abn$chl3=chlclim[,3]
# FData.abn$chl4=chlclim[,4]
# FData.abn$chl5=chlclim[,5]
# FData.abn$chl6=chlclim[,6]
# FData.abn$chl7=chlclim[,7]
# FData.abn$chl8=chlclim[,8]
# FData.abn$chl9=chlclim[,9]
# FData.abn$chl10=chlclim[,10]
# FData.abn$chl11=chlclim[,11]
# FData.abn$chl12=chlclim[,12]
# 
# FData.bio$chl1=chlclim[,1]
# FData.bio$chl2=chlclim[,2]
# FData.bio$chl3=chlclim[,3]
# FData.bio$chl4=chlclim[,4]
# FData.bio$chl5=chlclim[,5]
# FData.bio$chl6=chlclim[,6]
# FData.bio$chl7=chlclim[,7]
# FData.bio$chl8=chlclim[,8]
# FData.bio$chl9=chlclim[,9]
# FData.bio$chl10=chlclim[,10]
# FData.bio$chl11=chlclim[,11]
# FData.bio$chl12=chlclim[,12]
```
...
fish=FData.abn %>% dplyr::select(YEAR, SEASON, LAT:BOTTEMP, `73_Adt`, `73_Juv`, `73_ich`, calfin_100m3:chl12)
...
```{r}
zoo=FData.abn %>% dplyr::select(YEAR, SEASON, LAT:BOTSALIN, calfin_100m3:chl12)
...
zoo_modG_pa = gam(pseudo_100m3 ~ s(BOTTEMP, k=35, bs="tp") +s(SURFTEMP, k=35, bs='tp') + s(DEPTH, k=30, bs="tp") + s(BOTSALIN, k=30, bs='tp') + s(SURFSALIN, k=30, bs="tp") + s(grnszmm, k=30, bs='tp') + s(rug, k=30, bs='tp'), data=trainzoo, method = "REML", family="binomial")

zoo_modG_pb = gam(pseudo_100m3 ~ s(BOTTEMP, k=35, bs="tp") +s(SURFTEMP, k=35, bs='tp') + s(DEPTH, k=30, bs="tp") + s(BOTSALIN, k=30, bs='tp') + s(SURFSALIN, k=30, bs="tp") + s(grnszmm, k=30, bs='tp') + s(rug, k=30, bs='tp'), data=trainzoobio, method = "REML", family="gaussian")
...
# ______
zoo_modG4_pa = gam(pseudo_100m3 ~ s(LON, LAT, bs='tp') + s(BOTTEMP, k=35, bs="tp") +s(SURFTEMP, k=35, bs='tp') + s(DEPTH, k=30, bs="tp") + s(BOTSALIN, k=30, bs='tp') + s(SURFSALIN, k=30, bs="tp") + s(grnszmm, k=30, bs='tp') + s(rug, k=30, bs='tp'), data=trainzoo, method = "REML", family="binomial")

zoo_modG4_pb = gam(pseudo_100m3 ~ s(LON, LAT, bs='tp') + s(BOTTEMP, k=35, bs="tp") +s(SURFTEMP, k=35, bs='tp') + s(DEPTH, k=30, bs="tp") + s(BOTSALIN, k=30, bs='tp') + s(SURFSALIN, k=30, bs="tp") + s(grnszmm, k=30, bs='tp') + s(rug, k=30, bs='tp'), data=trainzoobio, method = "REML", family="gaussian")
...
zoo_modG4bll_pa = gam(pseudo_100m3 ~ s(LON, LAT, bs = "so", xt = list(bnd = boundll)) + s(BOTTEMP, k=20, bs="tp") +s(SURFTEMP, k=20, bs='tp') + s(DEPTH, k=10, bs="tp") + s(BOTSALIN, k=30, bs='tp') + s(SURFSALIN, k=30, bs="tp") + s(grnszmm, k=10, bs='tp') + s(rug, k=20, bs='tp'), data=trainzoo.sub, method = "REML", knots=knotsll, family="binomial")

zoo_modG4bll_pb = gam(pseudo_100m3 ~ s(LON, LAT, bs = "so", xt = list(bnd = boundll)) + s(BOTTEMP, k=20, bs="tp") +s(SURFTEMP, k=20, bs='tp') + s(DEPTH, k=10, bs="tp") + s(BOTSALIN, k=30, bs='tp') + s(SURFSALIN, k=30, bs="tp") + s(grnszmm, k=10, bs='tp') + s(rug, k=20, bs='tp'), data=trainzoo.subbio, method = "REML", knots=knotsll, family="gaussian")
...
```{r}
### select data
fish.b=FData.bio %>% dplyr::select(YEAR, SEASON, LAT:BOTTEMP, `73_Adt`, `73_Juv`, `73_ich`, calfin_100m3:chl12)
...
```{r}
fish_modG_pa = gam(pa ~ s(BOTTEMP, k=20, bs="tp") +s(SURFTEMP, k=20, bs='tp') + s(DEPTH, k=10, bs="tp") + s(Stg, k=3, bs='re') + s(pseudo_100m3, k=20, bs="cr") + s(grnszmm, k=10, bs='tp') + s(rug, k=20, bs='tp'), data=trainPA, method = "REML", family="binomial")

fish_modG_pb = gam(logbio ~ s(BOTTEMP, k=20, bs="tp") +s(SURFTEMP, k=20, bs='tp') + s(DEPTH, k=10, bs="tp") + s(Stg, k=3, bs='re') + s(pseudo_100m3, k=20, bs="cr") + s(grnszmm, k=10, bs='tp') + s(rug, k=20, bs='tp'), data=trainBIO, method = "REML", family="gaussian")
...
```{r}
fish_modG4_pa = gam(pa ~ s(LON, LAT) + s(BOTTEMP, k=20, bs="tp") +s(SURFTEMP, k=20, bs='tp') + s(DEPTH, k=10, bs="tp") + s(Stg, k=4, bs='re') + s(pseudo_100m3, k=20, bs="cr") + s(grnszmm, k=10, bs='tp') + s(rug, k=20, bs='tp'), data=trainPA, method = "REML", family="binomial")

fish_modG4_pb = gam(logbio ~ s(LON, LAT) + s(BOTTEMP, k=20, bs="tp") +s(SURFTEMP, k=20, bs='tp') + s(DEPTH, k=10, bs="tp") + s(Stg, k=4, bs='re') + s(pseudo_100m3, k=20, bs="cr") + s(grnszmm, k=10, bs='tp') + s(rug, k=20, bs='tp'), data=trainBIO, method = "REML", family="gaussian")
...
# draw(fish_modG4)
# [1] 16241.43
# pred.test=predict(fish_modG4_pa,testPA,type='response')                  # make prediction for test set
...
# gdepth[gdepth>0]=NA
# gdepth[gdepth< -5000]=NA
# # save(gdepth, file='/home/ryan/Git/NEhabitat/rasters/depth.RData')
...
fish_modG4bll_pa = gam(pa ~ s(LON, LAT, bs = "so", xt = list(bnd = boundll)) + s(BOTTEMP, k=20, bs="tp") +s(SURFTEMP, k=20, bs='tp') + s(DEPTH, k=10, bs="tp") + s(Stg, k=4, bs='fs') + s(pseudo_100m3, k=20, bs="cr") + s(grnszmm, k=10, bs='tp') + s(rug, k=20, bs='tp'), data=trainPA.sub, method = "REML", knots=knotsll, family="binomial")

fish_modG4bll_pb = gam(logbio ~ s(LON, LAT, bs = "so", xt = list(bnd = boundll)) + s(BOTTEMP, k=20, bs="tp") +s(SURFTEMP, k=20, bs='tp') + s(DEPTH, k=10, bs="tp") + s(Stg, k=4, bs='fs') + s(pseudo_100m3, k=20, bs="cr") + s(grnszmm, k=10, bs='tp') + s(rug, k=20, bs='tp'), data=trainBIO.sub, method = "REML", knots=knotsll, family="gaussian")
...
# draw(fish_modG4bll)
# 
## this works, just give it the training data and merge results
# fish_modG4bll_pred <- cbind(testPA,
#                        predict(fish_modG4bll, 
#                                testPA, 
#                                se.fit=TRUE, 
#                                type="response"))
...
# add fs for stg to all vars, keeping LAT and LON -> problematic(?)
...
# remove lat and lon
fish_modGSe_pa =  gam(pa ~ s(BOTTEMP, bs='tp') + s(BOTTEMP, Stg, k=10, bs="fs") + s(SURFTEMP, bs='tp') + s(SURFTEMP, Stg, k=10, bs='fs') + s(DEPTH, bs='tp') + s(DEPTH, Stg, k=10, bs="fs") + s(pseudo_100m3, bs='tp') + s(pseudo_100m3, Stg, k=10, bs="fs") + s(grnszmm, bs='tp') + s(grnszmm, Stg, k=10, bs='fs') + s(rug, bs='tp') + s(rug, Stg, k=10, bs='fs'), data=trainPA, method = "REML", family="binomial")

fish_modGSe_pb =  gam(logbio ~ s(BOTTEMP, bs='tp') + s(BOTTEMP, Stg, k=10, bs="fs") + s(SURFTEMP, bs='tp') + s(SURFTEMP, Stg, k=10, bs='fs') + s(DEPTH, bs='tp') + s(DEPTH, Stg, k=10, bs="fs") + s(pseudo_100m3, bs='tp') + s(pseudo_100m3, Stg, k=10, bs="fs") + s(grnszmm, bs='tp') + s(grnszmm, Stg, k=10, bs='fs') + s(rug, bs='tp') + s(rug, Stg, k=10, bs='fs'), data=trainBIO, method = "REML", family="gaussian")

# # no rugosity
fish_modGSe1_pa =  gam(pa ~ s(BOTTEMP, bs='tp') + s(BOTTEMP, Stg, k=10, bs="fs") + s(SURFTEMP, bs='tp') + s(SURFTEMP, Stg, k=10, bs='fs') + s(DEPTH, bs='tp') + s(DEPTH, Stg, k=10, bs="fs") + s(pseudo_100m3, bs='tp') + s(pseudo_100m3, Stg, k=10, bs="fs") + s(grnszmm, bs='tp') + s(grnszmm, Stg, k=10, bs='fs'), data=trainPA, method = "REML", family="binomial")
# no rugosity or grain size
fish_modGSe2_pa =  gam(pa ~ s(BOTTEMP, bs='tp') + s(BOTTEMP, Stg, k=10, bs="fs") + s(SURFTEMP, bs='tp') + s(SURFTEMP, Stg, k=10, bs='fs') + s(DEPTH, bs='tp') + s(DEPTH, Stg, k=10, bs="fs") + s(pseudo_100m3, bs='tp') + s(pseudo_100m3, Stg, k=10, bs="fs"), data=trainPA, method = "REML", family="binomial")
# no grain size
fish_modGSe3_pa =  gam(pa ~ s(BOTTEMP, bs='tp') + s(BOTTEMP, Stg, k=10, bs="fs") + s(SURFTEMP, bs='tp') + s(SURFTEMP, Stg, k=10, bs='fs') + s(DEPTH, bs='tp') + s(DEPTH, Stg, k=10, bs="fs") + s(pseudo_100m3, bs='tp') + s(pseudo_100m3, Stg, k=10, bs="fs") + s(rug, bs='tp') + s(rug, Stg, k=10, bs='fs'), data=trainPA, method = "REML", family="binomial")

# # no rugosity
fish_modGSe1_pb =  gam(logbio ~ s(BOTTEMP, bs='tp') + s(BOTTEMP, Stg, k=10, bs="fs") + s(SURFTEMP, bs='tp') + s(SURFTEMP, Stg, k=10, bs='fs') + s(DEPTH, bs='tp') + s(DEPTH, Stg, k=10, bs="fs") + s(pseudo_100m3, bs='tp') + s(pseudo_100m3, Stg, k=10, bs="fs") + s(grnszmm, bs='tp') + s(grnszmm, Stg, k=10, bs='fs'), data=trainBIO, method = "REML", family="gaussian")
# no rugosity or grain size
fish_modGSe2_pb =  gam(logbio ~ s(BOTTEMP, bs='tp') + s(BOTTEMP, Stg, k=10, bs="fs") + s(SURFTEMP, bs='tp') + s(SURFTEMP, Stg, k=10, bs='fs') + s(DEPTH, bs='tp') + s(DEPTH, Stg, k=10, bs="fs") + s(pseudo_100m3, bs='tp') + s(pseudo_100m3, Stg, k=10, bs="fs"), data=trainBIO, method = "REML", family="gaussian")
#no grain size
fish_modGSe3_pb =  gam(logbio ~ s(BOTTEMP, bs='tp') + s(BOTTEMP, Stg, k=10, bs="fs") + s(SURFTEMP, bs='tp') + s(SURFTEMP, Stg, k=10, bs='fs') + s(DEPTH, bs='tp') + s(DEPTH, Stg, k=10, bs="fs") + s(pseudo_100m3, bs='tp') + s(pseudo_100m3, Stg, k=10, bs="fs") + s(rug, bs='tp') + s(rug, Stg, k=10, bs='fs'), data=trainBIO, method = "REML", family="gaussian")

save(fish_modGSe3_pa, file=paste('/home/ryan/Git/NEhabitat/rasters/',fishseas,'/',fishname,'/fish_modGSe3_pa_',fishseas,'_',fishname,'.Rdata',sep=''))
save(fish_modGSe3_pb, file=paste('/home/ryan/Git/NEhabitat/rasters/',fishseas,'/',fishname,'/fish_modGSe3_pb_',fishseas,'_',fishname,'.Rdata',sep=''))
...
#check on order -> R,G,B = adt (1), ich (3), juv (2) (respectively) == 1,3,2 as factor values
```{r}
foo=evaluate_smooth(fish_modGSe_pa,"s(pseudo_100m3,Stg)")
...
```{r}
# fish_modGI4 =  gam(pa ~ s(LON, LAT, m=2, bs='tp') + s(LON, LAT, by=Stg, m=1, bs='tp') + s(BOTTEMP, k=10, m=2, bs="tp") + s(BOTTEMP, k=10, m=1 by=Stg, bs="tp") +s(SURFTEMP, Stg, k=10, m=2, bs='tp') + s(SURFTEMP, Stg, k=10, m=1, by=Stg, bs='tp') + s(DEPTH, Stg, k=10, m=2, bs="tp") + s(DEPTH, Stg, k=10, m=1, by=Stg, bs="tp") + s(pseudo_100m3, Stg, k=10, m=2, bs="tp") + s(pseudo_100m3, Stg, m=1, k=10, by=Stg, bs="tp")+ s(grnszmm, Stg, m=2, k=10, bs='tp') + s(grnszmm, Stg, k=10, m=1, by=Stg, bs='tp') + s(rug, Stg, m=2, k=10, bs='tp') +s(rug, Stg, k=10, m=1, by=Stg, bs='tp') + s(Stg, k=10, bs='re'), data=trainPA, method = "REML", family="binomial")

fish_modGI_pa =  gam(pa ~ s(BOTTEMP, k=10, m=2, bs="tp") + s(BOTTEMP, k=10, m=1, by=Stg, bs="tp") +s(SURFTEMP, k=10, m=2, bs='tp') + s(SURFTEMP, by=Stg, k=10, m=1, bs='tp') + s(DEPTH, k=10, m=2, bs="tp") + s(DEPTH, by=Stg, k=10, m=1, bs="tp") + s(pseudo_100m3, k=10, m=2, bs="tp") + s(pseudo_100m3, m=1, k=10, by=Stg, bs="tp")+ s(grnszmm, m=2, k=10, bs='tp') + s(grnszmm, by=Stg, k=10, m=1, bs='tp') + s(rug, m=2, k=10, bs='tp') +s(rug, by=Stg, k=10, m=1, bs='tp') + s(Stg, k=10, bs='re'), data=trainPA, method = "REML", family="binomial")

fish_modGI_pb =  gam(logbio ~ s(BOTTEMP, k=10, m=2, bs="tp") + s(BOTTEMP, k=10, m=1, by=Stg, bs="tp") +s(SURFTEMP, k=10, m=2, bs='tp') + s(SURFTEMP, by=Stg, k=10, m=1, bs='tp') + s(DEPTH, k=10, m=2, bs="tp") + s(DEPTH, by=Stg, k=10, m=1, bs="tp") + s(pseudo_100m3, k=10, m=2, bs="tp") + s(pseudo_100m3, m=1, k=10, by=Stg, bs="tp")+ s(grnszmm, m=2, k=10, bs='tp') + s(grnszmm, by=Stg, k=10, m=1, bs='tp') + s(rug, m=2, k=10, bs='tp') +s(rug, by=Stg, k=10, m=1, bs='tp') + s(Stg, k=10, bs='re'), data=trainBIO, method = "REML", family="gaussian")
...
```{r}
fish_modS_pa =  gam(pa ~ s(BOTTEMP, Stg, k=10, bs="fs") + s(SURFTEMP, Stg, k=10, bs='fs') + s(DEPTH, Stg, k=10, bs="fs") + s(pseudo_100m3, Stg, k=10, bs="fs") + s(grnszmm, Stg, k=10, bs='fs') + s(rug, Stg, k=10, bs='fs'), data=trainPA, method = "REML", family="binomial")

fish_modS_pb =  gam(logbio ~ s(BOTTEMP, Stg, k=10, bs="fs") + s(SURFTEMP, Stg, k=10, bs='fs') + s(DEPTH, Stg, k=10, bs="fs") + s(pseudo_100m3, Stg, k=10, bs="fs") + s(grnszmm, Stg, k=10, bs='fs') + s(rug, Stg, k=10, bs='fs'), data=trainBIO, method = "REML", family="gaussian")
...
```{r}
fish_modI_pa =  gam(pa ~ Stg + s(BOTTEMP, by=Stg, k=10, bs="tp") + s(SURFTEMP, by=Stg, k=10, bs='tp') + s(DEPTH, by=Stg, k=10, bs="tp") + s(pseudo_100m3, by=Stg, k=10, bs="tp") + s(grnszmm, by=Stg, k=10, bs='tp') + s(rug, by=Stg, k=10, bs='tp'), data=trainPA, method = "REML", family="binomial")

fish_modI_pb =  gam(logbio ~ Stg + s(BOTTEMP, by=Stg, k=10, bs="tp") + s(SURFTEMP, by=Stg, k=10, bs='tp') + s(DEPTH, by=Stg, k=10, bs="tp") + s(pseudo_100m3, by=Stg, k=10, bs="tp") + s(grnszmm, by=Stg, k=10, bs='tp') + s(rug, by=Stg, k=10, bs='tp'), data=trainBIO, method = "REML", family="gaussian")
