...
```{r}
fish_modG = gam(pa ~ s(BOTTEMP, k=20, bs="tp") +s(SURFTEMP, k=20, bs='tp') + s(DEPTH, k=10, bs="tp") + s(Stg, k=3, bs='re') + s(pseudo_100m3, k=20, bs="cr") + s(grnszmm, k=10, bs='tp') + s(rug, k=20, bs='tp'), data=trainPA, method = "REML", family="binomial")

# save(fish_modG, file='/home/ryan/Git/NEhabitat/rasters/Spr/Haddock/fish_modG_spr_haddock.Rdata')
# save(fish_modG, file='/home/ryan/Git/NEhabitat/rasters/Fall/Haddock/fish_modG_fall_haddock.Rdata')
...
fish_modG4bll = gam(pa ~ s(LON, LAT, bs = "so", xt = list(bnd = boundll)) + s(BOTTEMP, k=20, bs="tp") +s(SURFTEMP, k=20, bs='tp') + s(DEPTH, k=10, bs="tp") + s(Stg, k=4, bs='fs') + s(pseudo_100m3, k=20, bs="cr") + s(grnszmm, k=10, bs='tp') + s(rug, k=20, bs='tp'), data=trainPA.sub, method = "REML", knots=knotsll, family="binomial")

# save(fish_modG4bll, file='/home/ryan/Git/NEhabitat/rasters/Spr/Haddock/fish_modG4bll_spr_haddock.Rdata')
# save(fish_modG4bll, file='/home/ryan/Git/NEhabitat/rasters/Fall/Haddock/fish_modG4bll_fall_haddock.Rdata')
...
### this is too large -> need a different approach, see below for split data use
# fish_modG4bll_pred <- with(testPA, expand.grid(Lat=seq(min(LAT), max(LAT), length=10), Lon=seq(min(LON), max(LON), length=10), BT=seq(min(BOTTEMP), max(BOTTEMP), length=10), ST=seq(min(SURFTEMP), max(SURFTEMP), length=10), Z=seq(min(DEPTH), max(DEPTH), length=20), Zoo=seq(min(pseudo_100m3), max(pseudo_100m3), length=10), grn=seq(min(grnszmm), max(grnszmm), length=10), rug=seq(min(rug), max(rug), length=5), Stg=levels(Stg)))
...
# remove lat and lon
fish_modGSe =  gam(pa ~ s(BOTTEMP, bs='tp') + s(BOTTEMP, Stg, k=10, bs="fs") + s(SURFTEMP, bs='tp') + s(SURFTEMP, Stg, k=10, bs='fs') + s(DEPTH, bs='tp') + s(DEPTH, Stg, k=10, bs="fs") + s(pseudo_100m3, bs='tp') + s(pseudo_100m3, Stg, k=10, bs="fs") + s(grnszmm, bs='tp') + s(grnszmm, Stg, k=10, bs='fs') + s(rug, bs='tp') + s(rug, Stg, k=10, bs='fs'), data=trainPA, method = "REML", family="binomial")

# save(fish_modGSe, file='/home/ryan/Git/NEhabitat/rasters/Spr/Haddock/fish_modGSe_spr_haddock.Rdata')
# save(fish_modGSe, file='/home/ryan/Git/NEhabitat/rasters/Fall/Haddock/fish_modGSe_fall_haddock.Rdata')
...
```{r}
fish_modS =  gam(pa ~ s(BOTTEMP, Stg, k=10, bs="fs") + s(SURFTEMP, Stg, k=10, bs='fs') + s(DEPTH, Stg, k=10, bs="fs") + s(pseudo_100m3, Stg, k=10, bs="fs") + s(grnszmm, Stg, k=10, bs='fs') + s(rug, Stg, k=10, bs='fs'), data=trainPA, method = "REML", family="binomial")

# save(fish_modS, file='/home/ryan/Git/NEhabitat/rasters/Spr/Haddock/fish_modS_spr_haddock.Rdata')
# save(fish_modS, file='/home/ryan/Git/NEhabitat/rasters/Fall/Haddock/fish_modS_fall_haddock.Rdata')
```

I model
y∼fac+s(x, by=fac) or y∼fac+te(x1,x2, by=fac)
```{r}
fish_modI =  gam(pa ~ Stg + s(BOTTEMP, by=Stg, k=10, bs="tp") + s(SURFTEMP, by=Stg, k=10, bs='tp') + s(DEPTH, by=Stg, k=10, bs="tp") + s(pseudo_100m3, by=Stg, k=10, bs="tp") + s(grnszmm, by=Stg, k=10, bs='tp') + s(rug, by=Stg, k=10, bs='tp'), data=trainPA, method = "REML", family="binomial")

# save(fish_modI, file='/home/ryan/Git/NEhabitat/rasters/Spr/Haddock/fish_modI_spr_haddock.Rdata')
# save(fish_modI, file='/home/ryan/Git/NEhabitat/rasters/Fall/Haddock/fish_modI_fall_haddock.Rdata')
```
