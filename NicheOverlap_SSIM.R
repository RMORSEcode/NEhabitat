...
tadtssim=as.matrix(unlist(adtno))
plot(tadtssim~yrlist, type='b', main='Adt SSIM')

juvno=data.frame(matrix(NA, ncol=1, nrow=dim(adt1)[3])); colnames(juvno)='SSIM'
for( i in 1:dim(adt1)[3]){
  r1m = rasterToPoints(juv1[[i]])
  r2m = rasterToPoints(juv2[[i]])
  juvno[i,1]=SSIM(r1m[,3], r2m[,3], numBreaks = 256)
}
tjuvssim=as.matrix(unlist(juvno))
plot(tjuvssim~yrlist, type='b', main='Juv SSIM')

ichno=data.frame(matrix(NA, ncol=1, nrow=dim(adt1)[3])); colnames(ichno)='SSIM'
for( i in 1:dim(adt1)[3]){
  r1m = rasterToPoints(ich1[[i]])
  r2m = rasterToPoints(ich2[[i]])
  juvno[i,1]=SSIM(r1m[,3], r2m[,3], numBreaks = 256)
}
tichssim=as.matrix(unlist(juvno))
